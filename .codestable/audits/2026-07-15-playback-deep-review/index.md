...
HLS 缓冲、关闭预取、移除父级 `currentTime` 回灌等历史优化仍有效。本轮对 8 条 finding 完成了定点修复与复核：#1-#6 已解决，#7、#8 保持 open。同路由推荐跳转、后台补源旧快照、换源 hydrate 离页提交、same-URL 换源复用旧播放器、旧初始化晚到、弹幕/跳过配置旧结果覆盖、单进程播放记录乱序和常驻 RAF 均已收口。
...
| #3      | resolved | initializer 增加 generation/cleanup，每个 await 后校验；已提交播放器事件改用 player identity；source/id 成为 effect identity，同 URL 换源也会重新初始化并重建旧播放器。 |
...
| #7      | open     | 弹幕请求生命周期已修复，但“无偏好默认 true”的产品策略与重复偏好读取未改。                                                                                               |
...
- 本地生产包浏览器冒烟确认登录页可渲染、`/play?...` 认证重定向保留完整参数且控制台无 error；因无本地登录态且远端 Redis 不可达，未完成登录后播放器目视验收。

## 下一步建议

- **仓库基线另行治理**：全仓 ESLint warning 与 Prettier 历史债务不属于本轮首页/播放页修复范围。
- **产品策略确认后处理**：#7 需要明确新用户默认是否加载弹幕，再统一初始化与偏好读取。

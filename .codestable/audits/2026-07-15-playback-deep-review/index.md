...
| #8      | open     | 已删只清不写 refs 与 source/favorite reducer 假 owner，合并重复 ended 注册并补生命周期测试；大文件职责、多套 loading owner 和统一播放状态机仍未拆。           |
...
- 最新门禁：`pnpm exec jest --runInBand` 通过（83 suites / 441 tests），`pnpm typecheck`、`pnpm build`、本批播放文件的 `eslint --max-warnings=0` 与 `git diff --check` 通过；production build 通过本机 SSH tunnel 复用 `136.175.83.3` 回环 Redis，构建后已关闭 tunnel。

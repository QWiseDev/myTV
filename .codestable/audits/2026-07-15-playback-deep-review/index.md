...
- 最新门禁：`pnpm exec jest --runInBand` 通过（69 suites / 317 tests），`pnpm typecheck`、`pnpm build`、本轮全部 TS/TSX 文件的 `eslint --max-warnings=0` 与 `git diff --check` 通过；production build 通过本机 SSH tunnel 复用 `136.175.83.3` Redis，构建后已关闭 tunnel。

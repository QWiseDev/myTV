...
旧首页审计的 7 条修复当前均未回退。本轮对 9 条 finding 完成了定点修复与复核：#1-#5、#8 已解决，#7 为 partially-resolved，#6、#9 保持 open。分页请求交叠、clear-all 在途请求、TV/综艺部分数据覆盖、Bangumi 异常响应、第二页续播缺失、收藏补全丢更新和 image-proxy 测试基线均已收口；`VideoCard` 的图片代理监听、未打开 ActionSheet 成本和收藏查询/事件竞态也已修复。

剩余 2 条 open 与 1 条 partially-resolved 均涉及策略或更大范围的行为等价重构：冷缓存 SSR 仍等待低优先级数据，卡片仍保留较重的按实例状态，首页 fallback/Suspense 所有权仍未完全收敛。审计目录保持 `active`，直到这些 finding 被处理或明确接受。
...
| 6   | open               | performance     | P1     | high   | 冷缓存首屏等待最慢三级数据              | [finding-06.md](finding-06.md) |
...
| #6      | open               | 本轮未改变服务端 `Promise.all`、SSR 总预算和部分聚合缓存策略。                                                                                                                                          |
...
- 最终门禁：`pnpm exec jest --runInBand` 通过（62 suites / 252 tests），`pnpm typecheck`、`pnpm build`、本轮全部 TS/TSX 文件的 `eslint --max-warnings=0` 与 `git diff --check` 通过。
- 仓库级 `pnpm lint:strict` 仍被 147 条既有 warning 阻断；`pnpm format:check` 仍命中数百个历史未格式化文件。本轮新增测试与 CodeStable 文档已单独通过 Prettier。
- 本地生产包浏览器冒烟确认登录页可渲染、`/` 与 `/play?...` 认证重定向正确且控制台无 error；因无本地登录态且远端 Redis 不可达，未完成登录后首页/播放页目视验收。

## 下一步建议

- **P1 首屏策略单独设计**：#6 会改变 SSR 阻塞边界与缓存策略，不应与已完成的 bugfix 混改。

...
旧首页审计的 7 条修复当前均未回退。本轮 9 条 finding 已全部完成定点修复与复核。分页请求交叠、clear-all 在途请求、TV/综艺部分数据覆盖、Bangumi 异常响应、第二页续播缺失、收藏补全丢更新、冷缓存 SSR/客户端聚合阻塞、image-proxy 测试基线和 fallback/Suspense 重复均已收口；`VideoCard` 的 props 镜像、图片代理与收藏更新底层监听、未打开 timer 和关闭后隐藏 ActionSheet 实例也已移除。

普通收藏与搜索收藏仍保留各自 hook，因为两者分别承担延迟持续同步和按需三态加载，并非同一行为的重复实现；没有 profile 证据时不继续扩大重构范围。审计目录状态更新为 `completed`。
...
| 7   | resolved | performance     | P2     | high   | 每张 VideoCard 都挂载完整状态与全局监听 | [finding-07.md](finding-07.md) |
...
| #7      | resolved | 图片代理配置与 `favoritesUpdated` 底层订阅已共享，`VideoCardHandle/useSyncedState` 与搜索 ref/cache 补丁已删除；ActionSheet 关闭后会在 200ms 退场结束时真正卸载，重新打开和提前卸载均有生命周期门禁。                  |
...
- 已知阶段性结果：首页相关分组 9 个 suites / 30 个 tests 通过；`useFavoriteItems.test.ts` 5 个 tests 通过；`usePlaybackRecords.test.ts` 的分页交叠定向用例曾独立通过；最新 `VideoCard.test.tsx` 15 个 tests、目标 ESLint 和 typecheck 通过。
...
- 最终门禁：`pnpm exec jest --runInBand` 通过（68 suites / 291 tests），`pnpm typecheck`、`pnpm build`、本轮全部 TS/TSX 文件的 `eslint --max-warnings=0` 与 `git diff --check` 通过；本轮新增测试单独通过 Prettier。
...
- **保持当前收藏边界**：普通收藏/搜索收藏两套状态语义不同；只有后续 profile 证明每卡 state 本身仍构成瓶颈时，再单独设计合并方案。

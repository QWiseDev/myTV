...
// 远程版本检查URL配置 - 已禁用网络请求
const _VERSION_CHECK_URLS: string[] = [];
 ...
 */
async function _fetchVersionFromUrl(_url: string): Promise<string | null> {
  try {
    ...
    const urlWithTimestamp = _url.includes('?')
      ? `${_url}&_t=${timestamp}`
      : `${_url}?_t=${timestamp}`;
    ...
    console.warn(`从 ${_url} 获取版本信息失败:`, error);
    return null;
  }
}

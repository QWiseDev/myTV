...
async function checkRateLimit(
  ...
  const key = `rate:tvbox:${ip}:${windowStart}`;
  const expireSeconds = Math.ceil(windowMs / 1000); // 转换为秒

  try {
    const redisCount = await db.incrementRateLimit(key, expireSeconds);
    if (redisCount !== null) {
      return redisCount <= limit;
    }

    const cacheKey = `tvbox-rate-limit:${ip}:${windowStart}`;
    // 获取当前计数
    const currentCount = (await db.getCache(cacheKey)) || 0;
    ...
    await db.setCache(cacheKey, newCount, expireSeconds);
...
export async function GET(request: NextRequest) {
  try {
    ...
    let tvboxConfig: TVBoxConfig = {
      ...
      wallpaper: `${baseUrl}/logo.svg`, // 使用项目Logo作为壁纸

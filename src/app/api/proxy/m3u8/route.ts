  ...
  try {
    ...
    if (isM3U8) {
      ...
      const filteredContent = filterAdsFromM3U8(m3u8Content, {
        type: source || '',
        customCode: config.SiteConfig?.CustomAdFilterCode || '',
        onCustomError: (error) => {
          console.error('执行自定义去广告代码失败，使用默认规则:', error);
        },
      });
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line && !line.startsWith('#')) {
      ...
      const proxyUrl = allowCORS
        ? resolvedUrl
        : createProxyUrl(proxyBase, 'segment', resolvedUrl, source);
...
function createProxyUrl(
  ...
  return `${proxyBase}/${endpoint}?${params.toString()}`;
}
...
function rewriteMapUri(
  ...
  if (uriMatch) {
    ...
    const proxyUrl = createProxyUrl(proxyBase, 'segment', resolvedUrl, source);
...
function rewriteKeyUri(
  ...
  if (uriMatch) {
    ...
    const resolvedUrl = resolveUrl(baseUrl, originalUri);
    const proxyUrl = createProxyUrl(proxyBase, 'key', resolvedUrl, source);
    return line.replace(uriMatch[0], `URI="${proxyUrl}"`);
  }
  return line;
}
...
function rewriteMediaUri(
  ...
  if (uriMatch) {
    ...
    try {
      const resolvedUrl = resolveUrl(baseUrl, originalUri);
      const proxyUrl = createProxyUrl(proxyBase, 'm3u8', resolvedUrl, source);
      return line.replace(uriMatch[0], `URI="${proxyUrl}"`);
    } catch (error) {
      if (process.env.NODE_ENV === 'development') {

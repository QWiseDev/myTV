        ...
        celebrityItems.forEach((item) => {
          ...
          if (linkMatch && avatarUrl) {
            ...
            if (!isDefaultAvatar) {
              celebrities.push({
                id: linkMatch[2],
                name: linkMatch[3].split(' ')[0], // 只取中文名
                avatar: avatarUrl,
    ...
    if (recommendationsSection) {
      ...
      if (recommendItems) {
        recommendItems.forEach((item) => {
          ...
          // 提取标题
          const itemTitleMatch = item.match(/alt="([^"]+)"/);
          // 提取海报
          const itemPosterMatch = item.match(/<img src="([^"]+)"/);
          // 提取评分
          const rateMatch = item.match(
            /<span class="subject-rate">([^<]+)<\/span>/
          );

          if (idMatch && itemTitleMatch && itemPosterMatch) {
            recommendations.push({
              id: idMatch[1],
              title: itemTitleMatch[1],
              poster: itemPosterMatch[1],

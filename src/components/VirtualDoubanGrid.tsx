...
export const VirtualDoubanGrid: React.FC<VirtualDoubanGridProps> = ({
  ...
  }, [isVirtualLoadingMore, totalItemCount, hasMore, isLoadingMore]);

  // 网格行数计算 - 必须在useEffect之前声明
  ...
  // 处理单元渲染事件
  const handleCellsRendered = useCallback((visibleCells: any) => {
    const { rowStopIndex: visibleRowStopIndex } = visibleCells;
    visibleRowIndexRef.current = visibleRowStopIndex;
  }, []);

  // 使用 useEffect 监听可见区域变化并触发加载
  useEffect(() => {
    const visibleRowIndex = visibleRowIndexRef.current;
    if (visibleRowIndex === -1) return;

    if (visibleRowIndex >= rowCount - LOAD_MORE_THRESHOLD) {
